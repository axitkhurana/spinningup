...
def mlp_actor_critic(x, hidden_sizes=(64, 64), activation=tf.nn.relu,
    ...
    with tf.variable_scope(scope):
        ...
        common = mlp(x, list(hidden_sizes), activation)
        value = mlp(common, (32, 1), activation=tf.nn.relu)
        advantage = mlp(common, (32, act_dim), activation=tf.nn.relu)
        q = tf.tile(value, [1, 2]) + tf.subtract(advantage, tf.reduce_mean(advantage, axis=1, keepdims=True))
    return q
