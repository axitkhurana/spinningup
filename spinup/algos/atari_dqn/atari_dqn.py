...
import gym
import time
from spinup.algos.atari_dqn import core
from spinup.algos.atari_dqn.core import get_vars
...
"""
def atari_dqn(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
              steps_per_epoch=500, epochs=100, replay_size=int(1e6), gamma=0.99,
              epsilon_start=1, epsilon_step=1e-6, epsilon_end=0.1,
              q_lr=1e-3, batch_size=100, start_steps=5000, skip_frames=4,
    ...
    x_ph, x2_ph = core.placeholder_shape(obs_dim), core.placeholder_shape(obs_dim)
    ...
    def get_action(obs, eps):
        if np.random.random() < eps:
            ...
            q_vals = sess.run(q, feed_dict={x_ph: np.expand_dims(obs, axis=0)})
            a = np.squeeze(np.argmax(q_vals, axis=1))
        return a

    def test_agent(n=10):
        for j in range(n):
            o, r, d, ep_ret, ep_len = core.reset(test_env, skip_frames), 0, False, 0, 0
            while not(d or (ep_len == max_ep_len)):
                # Take deterministic actions at test time (noise_scale=0)
                o, r, d, _ = core.step(test_env, skip_frames, get_action(o, 0))
                ep_ret += r
                ep_len += 1
            logger.store(TestEpRet=ep_ret, TestEpLen=ep_len)

    start_time = time.time()
    o, r, d, ep_ret, ep_len = core.reset(env, skip_frames), 0, False, 0, 0
    ...
    for t in range(total_steps):
        ...
        # Step the env
        o2, r, d, _ = core.step(env, skip_frames, a)
        ep_ret += r
        ep_len += 1
...
if __name__ == '__main__':
    ...
    atari_dqn(lambda : gym.make(args.env), actor_critic=core.mlp_actor_critic,
