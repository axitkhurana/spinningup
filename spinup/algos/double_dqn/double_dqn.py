...
import gym
import time
from spinup.algos.double_dqn import core
from spinup.algos.double_dqn.core import get_vars, copy_operation
...
"""
def double_dqn(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
    ...
    # get action using q, and then calculate q_target_a
    a_using_q = tf.argmax(q, axis=1)
    qa_target = r_ph + gamma*(1-d_ph)*tf.reduce_sum(tf.one_hot(a_using_q, depth=env.action_space.n)*q_target, axis=1)
    # tf.stop_gradient(tf.reduce_max(q_target, axis=1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='double_dqn')
    args = parser.parse_args()

    from spinup.utils.run_utils import setup_logger_kwargs
    logger_kwargs = setup_logger_kwargs(args.exp_name, args.seed)

    double_dqn(lambda : gym.make(args.env), actor_critic=core.mlp_actor_critic,
